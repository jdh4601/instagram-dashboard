...
export function ChatPanel() {
  const chat = useChat();
  // xl 이상에서는 패널이 늘 열려 있고, 이 상태는 좁은 화면에서만 쓰인다.
  ...
  return (
    <>
      {/* 좁은 화면에서 접혀 있을 때 오른쪽 가장자리에 남는 손잡이 */}
      {!expanded && (
        <button
          type="button"
          onClick={() => setExpanded(true)}
          aria-label="계정 진단 AI 열기"
          className="fixed right-0 top-1/2 z-30 flex -translate-y-1/2 items-center gap-1.5 rounded-l-xl border border-r-0 border-border-subtle bg-surface py-3 pl-2 pr-1.5 shadow-card-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 xl:hidden"
        >
      ...
      <aside
        aria-label="계정 진단 AI"
        className={`w-full max-w-[26rem] flex-col border-l border-border-subtle bg-surface shadow-card-hover xl:sticky xl:top-0 xl:z-0 xl:flex xl:h-screen xl:w-[26rem] xl:max-w-none xl:shrink-0 xl:shadow-none ${
          expanded ? "fixed inset-y-0 right-0 z-40 flex" : "hidden"
        }`}
      >
        <header className="flex items-center justify-between gap-2 border-b border-border-subtle px-4 py-3">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            {/* 접기는 좁은 화면 전용이다. xl 이상에서는 패널이 늘 열려 있다. */}
            <button
              type="button"
              onClick={() => setExpanded(false)}
              aria-label="패널 접기"
