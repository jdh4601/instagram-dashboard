...
- 토큰은 약 60일 후 만료되지만, **대시보드가 알아서 갱신한다.** 동기화 버튼을 누를 때와 일일
  리포트 크론이 돌 때마다 남은 수명을 확인해서, **15일 미만이 남았으면 `refresh_access_token`으로
  60일을 연장하고 새 토큰을 `/settings`에 저장**한다. 갱신이 실패해도 동기화 자체는 계속되며,
  사유는 동기화 결과의 `errors`에 남는다.
- 다만 **두 경로 중 최소 하나는 60일 안에 한 번은 실행돼야 한다.** 대시보드를 두 달 넘게 열지
  않고 크론도 걸어 두지 않으면 토큰이 만료되고, **만료된 토큰은 갱신으로 되살릴 수 없다.**
  크론 설치 방법은 `scripts/launchd/README.md`에 있다.
- 이미 만료됐다면 설정 화면에서 토큰을 새로 생성하고 다시 장기 토큰으로 교환한 뒤 `/settings`에
  붙여넣는다. 이때부터는 만료 시각이 기록돼 자동 갱신이 판단할 근거가 생긴다.
